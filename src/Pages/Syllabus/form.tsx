...
const SyllabusForm: React.FC<SyllabusFormProps> = ({ handleCancel, sheet }) => {
  const dispatch = useDispatch();
  const [data, setData] = useState({});
  const { data: singleChildren } = useGetSingleSyllabusQuery(sheet?.id, {
    skip: !sheet?.id,
  });
  ...
  return (
    <div className="">
      <SheetHeader title={sheet?.id ? "Edit Syllabus" : "Add New Syllabus"} />
